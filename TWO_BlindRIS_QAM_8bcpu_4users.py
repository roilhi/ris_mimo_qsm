...
for j, snr in enumerate(SNR_l):
    ...
    for u in range(N_iter):
        ...
        Heq = H_RD1 @ H_SR1 + H_RD2 @ H_SR2 + H_SD
        ...
        # Rx from user 1
        S_y2 = (1/At)*(H11 @ Tx2) + (1/At)*(H12 @ Tx3) + H_SD1 @ Tx
        y2 = S_y2 + noise
        aptitud += np.linalg.norm(S_y2)
        C2 = H1 @ W1
        C = SS @ C2.T
        # ML detector
...
ax.set_ylabel('ABEP')
ax.set_title('ABEP 2 RIS 4 users 8 bcpu QAM modulation')
plt.grid()
plt.show()
